...
export function LiveOverview() {
  ...
  return <div className="ats-page overview-page">
    ...
    <div className="overview-grid"><MarketPanel />
      ...
      <Panel title="Live opportunities" eyebrow="R10 / R10-X FUNNEL" actions={<Link className="panel-link" href="/candidates">Open scanner →</Link>} className="opportunity-summary"><div className="funnel-mini"><div><strong>{pipeline?.candidates_considered ?? 0}</strong><span>Observed</span></div><i aria-hidden="true" /><div><strong>{Math.max(0, (pipeline?.candidates_considered ?? 0) - (pipeline?.candidates_qualified ?? 0))}</strong><span>Rejected</span></div><i aria-hidden="true" /><div className="qualified"><strong>{pipeline?.candidates_qualified ?? 0}</strong><span>Qualified</span></div></div>{!pipeline?.candidates_qualified ? <EmptyState title="No qualifying live opportunities" detail="The current pipeline has not produced a candidate that clears all governed checks." metrics={<span>{pipeline?.candidates_considered ?? 0} contracts considered · reasons remain in recorded evidence</span>} /> : <p className="calm-copy">Qualified candidates are awaiting or progressing through Portfolio Brain and A04 authority.</p>}</Panel>
