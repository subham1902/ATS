...
def test_entry_through_production_authority() -> None:
    ...
    rt = TradingRuntime(
        config=RuntimeConfig(calendar=cal), market_feed=feed, broker=broker, authority=svc
    )
...
def test_unknown_submit_holds_reservation_through_engine() -> None:
    ...
    feed.set_mark("NIFTY", Decimal("101"), now)
    rt = TradingRuntime(
        config=RuntimeConfig(calendar=cal, authority_reservation_amount=Decimal("10000")),
        market_feed=feed,
        broker=broker,
        authority=svc,
    )
...
def test_multi_position_through_engine_real_authority() -> None:
    ...
    rt = TradingRuntime(
        config=RuntimeConfig(calendar=cal, authority_reservation_amount=Decimal("80000")),
        market_feed=feed,
        broker=broker,
        authority=svc,
    )
...
def test_session_flatten_through_engine_with_authority() -> None:
    ...
    result = rt.process_event(
        RuntimeEvent(kind=RuntimeEventKind.BAR, instrument_id="NIFTY", payload={}, at=flatten)
    )
