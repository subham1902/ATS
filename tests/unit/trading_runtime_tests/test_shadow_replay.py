...
def test_walk_forward_multi_session_determinism() -> None:
    ...
    # With no repository-governed calibration store, the replay cannot force
    # trades or load machine-local evidence.
    assert res1["ending_equity"] == "100000"
    assert all(day["visible_cal_observations"] == 0 for day in res1["daily_ledger"])
