...
HOLDOUT_SESSIONS = ALL_SESSIONS[17:]       # Aug 27 - Aug 28 (2 sessions, strictly unread until final eval)

REPO_ROOT = Path(__file__).resolve().parents[1]
DATA_ROOT = Path(os.environ.get("ATS_SESSION_DATA_ROOT", str(REPO_ROOT / "data" / "raw" / "upstox" / "sessions")))
OUTPUT_DIR = Path(os.environ.get("ATS_TOURNAMENT_OUTPUT_DIR", str(REPO_ROOT / "data" / "replays" / "champion_replacement_tournament_v2")))
...
def load_session_bars(session_str: str, underlying: str) -> list[Bar5m]:
    ...
    # Raw candles are in reverse chronological order: [timestamp, open, high, low, close, volume, oi]
    ...
    for und in ["NIFTY", "BANKNIFTY"]:
        for s in sessions:
            ...
            for i in range(5, len(bars) - 5):
                ...
                rets_5 = [(bars[k].close - bars[k-1].close) / bars[k-1].close for k in range(i-4, i+1)]
...
class ModelM1(ModelFamily):
    def __init__(self):
        super().__init__("M1", "Challenger M1 (Regularized Logistic)", "L2 regularized multi-horizon logistic model")

    def fit_train(self, train_obs: list[Observation]) -> None:
        self.weights = {"roc_1": 15.0, "roc_3": 35.0, "roc_5": 20.0, "accel": 10.0, "vwap_dist_bps": 0.002}
        self.intercept = 0.0
...
class ModelM2(ModelFamily):
    def __init__(self):
        super().__init__("M2", "Challenger M2 (Robust Logit)", "Log-odds scaled by realized volatility")
...
class ModelM3(ModelFamily):
    def __init__(self):
        super().__init__("M3", "Challenger M3 (Trend Ensemble)", "Weighted short, mid, and long momentum blend")
...
class ModelM4(ModelFamily):
    def __init__(self):
        super().__init__("M4", "Challenger M4 (Regime Logistic)", "Trend-following in TREND; Mean-reverting in RANGE")
...
class ModelM5(ModelFamily):
    def __init__(self):
        super().__init__("M5", "Challenger M5 (Range Mean Reversion)", "Selective oscillator active only in RANGE regimes")
...
class ModelM6(ModelFamily):
    def __init__(self):
        super().__init__("M6", "Challenger M6 (Volatility Expansion)", "High-momentum breakout detector under volatility expansion")
...
class ModelM7(ModelFamily):
    def __init__(self):
        super().__init__("M7", "Challenger M7 (Cost-Aware Net EV)", "Direct probability of clearing realistic transaction costs")
...
class ModelM8(ModelFamily):
    def __init__(self):
        super().__init__("M8", "Challenger M8 (R10-X Convexity)", "Second-order acceleration and convexity signal")
...
class ModelM9(ModelFamily):
    def __init__(self):
        super().__init__("M9", "Challenger M9 (Mixture of Experts)", "Gated ensemble of M1, M4, and M7")
        self.m1 = ModelM1()
        self.m4 = ModelM4()
        self.m7 = ModelM7()
...
def compute_calibration_metrics(probs: list[float], targets: list[int]) -> dict[str, float]:
    ...
    for b in range(num_bins):
        ...
        indices = [idx for idx, p in enumerate(probs) if low <= p < high or (b == num_bins - 1 and p == high)]
        if indices:
...
def simulate_trades(
    ...
    *,
    ...
    profit_factor = total_win_amt / total_loss_amt if total_loss_amt > 0 else (99.0 if total_win_amt > 0 else 0.0)
    ...
    std_pnl = math.sqrt(sum((p - net_expectancy)**2 for p in pnl_series) / total_trades) if total_trades > 1 else 1.0
...
def run_tournament() -> dict[str, Any]:
    ...
    print(f"  Walk-Forward ({len(WALK_FORWARD_SESSIONS)}): {WALK_FORWARD_SESSIONS[0]} -> {WALK_FORWARD_SESSIONS[-1]}")
    print(f"  Untouched Holdout ({len(HOLDOUT_SESSIONS)}): {HOLDOUT_SESSIONS[0]} -> {HOLDOUT_SESSIONS[-1]}")
    print("=" * 70)
    ...
    print(f"Observations: Train={len(train_obs)}, Val={len(val_obs)}, Walk-Forward={len(wf_obs)}, Holdout={len(holdout_obs)}")
    ...
    for m in ALL_MODELS:
        ...
        passes_gates = (
            sc["validation_1x"]["net_pnl"] >= 0 and
            sc["validation_1_5x_stress"]["net_pnl"] >= 0 and
            sc["walk_forward_1x"]["net_pnl"] >= 0 and
            sc["walk_forward_1_5x_stress"]["net_pnl"] >= 0 and
            wf_trades >= 4 and
            brier < 0.30
        )
    ...
    promotion_verdict = (
        "NEW_CHAMPION_READY_FOR_SHADOW" if final_promotion_passed and top_model.model_id != "C0"
