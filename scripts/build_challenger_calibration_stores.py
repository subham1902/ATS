"""Generate model-specific, leakage-safe empirical calibration stores for every challenger."""
    ...
    for session_date in PRIOR_SESSIONS:
        for und in ["NIFTY", "BANKNIFTY"]:
            ...
            for i in range(4, len(snaps) - 3):
                ...
                # Evaluate model probability strictly from time T features
                raw_prob = prob_fn(roc, vol_feat)
                prob_dec = Decimal(str(round(min(0.9999, max(0.0001, raw_prob)), 4)))
...
def main() -> None:
    CALIBRATION_STORES_DIR.mkdir(parents=True, exist_ok=True)

    models: dict[str, Callable[[float, float], float]] = {
        "C0": lambda roc, vol: 0.50 + roc * 5.0,
        "A1": lambda roc, vol: 0.50 + roc * 10.0,
        "A2": lambda roc, vol: 0.50 + roc * 15.0,
        "A3": lambda roc, vol: 0.50 + roc * 20.0,
        ...
        "C5": lambda roc, vol: 0.50 + roc * (22.0 if vol > 0.005 else 12.0),
    }

    for model_id, fn in models.items():
        obs = build_model_calibration_history(fn)
        out_path = CALIBRATION_STORES_DIR / f"{model_id}_calibration_store.json"
        serialized = [o.model_dump(mode="json") for o in obs]
        out_path.write_text(json.dumps(serialized, indent=2), encoding="utf-8")
        print(f"Generated {len(obs)} calibration records for {model_id} -> {out_path}")
